...
class Controller{
	...
	private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false){
		...
		$is_plugin=isset($GLOBALS[PLUGIN_ID])&&!(!defined('STYLE_URL')&&defined('ADMIN_INI'));
		if($status){
			!isset($this->waitSecond) && $this->assign('waitSecond', 1000);
			!isset($this->jumpUrl) && $this->assign('jumpUrl', 'auto');
			$this->display(C('TMPL_ACTION_SUCCESS', $is_plugin?'message':'/message'));
		}else{
			$this->assign('error', $message);
			!isset($this->waitSecond) && $this->assign('waitSecond', 3000);
			!isset($this->jumpUrl) && $this->assign('jumpUrl', 'auto');
			$this->display(C('TMPL_ACTION_ERROR', $is_plugin?'message':'/message'));
		}
		exit();
	}
	 ...
	 * @param string $type
	 */
	public static function checkStatus($type='site'){
		$SITE=& getLcache('setting', 'setting', 'array', 'web');
		if(isset($SITE['status']) && !$SITE['status']){
			self::showmessage($SITE['notice'], 'auto');
			exit(0);
		}elseif(isset($SITE['app']) && $type=='app' && !$SITE['app']){
			exit(0);
		}
	}
	 ...
	 */
	public static function showmessage($message,$jumpUrl='goback',$waitSecond=1250,$dialog='',$returnjs=''){
		$is_admin=defined('ADMIN_INI');
		$filename=isset($GLOBALS[PLUGIN_ID]) ? template('message','admin') : template('message',$is_admin);
		if(is_file($filename)){
			if($is_admin){
				$SITE=& getLcache('setting', 'setting', 'array', 'web');
			}
