...
class ExcelFileParser{
	...
	var $sfat;
	/*
	 * Removed: var $sbd; Removed: var $syear;
	 */
	var $formats;
	var $xf;
	var $fonts;
	...
	function isDateFormat($val){
		...
		if(preg_match("/[m|d|y]/i", $this->format[$f_i]) != 0){
			if(strrpos($this->format[$f_i], '[') != FALSE){
				...
				return (preg_match("/[m|d|y]/i", $tmp) != 0) ? TRUE : FALSE;
			}else{
				return TRUE;
			}
		}else{
	...
	function getUnicodeString($str,$ofs){
		...
		$i_ofs=0;
		/*
		 * if(ord($str[$ofs])==255){ $size=ord($str[$ofs])+ 256*(ord($str[$ofs+1])); $i_ofs=2; } else {
		 */
		$size=ord($str[$ofs]);
		$i_ofs=1;
		/* } */
		...
		while((ord($ws[$ptr]) != 0x0a) && ($ptr < strlen($ws))){
			switch(ord($ws[$ptr]) + 256 * ord($ws[$ptr + 1])){
				...
				case 0x0204:
					...
					$data['cell'][$row][$col]['font']=$fonti;
					/*
					 * echo str_replace("\n","<br>\n",ExcelFont::toString($font,$fonti)); echo "FontRecord for sting ".$this->sst['data'][$sst_ind]."<br>";
					 */
					if(!isset($data['max_row']) || ($data['max_row'] < $row))
						$data['max_row']=$row;
					if(!isset($data['max_col']) || ($data['max_col'] < $col))
				...
				case 0x00fd:
					...
					$data['cell'][$row][$col]['font']=$fonti;
					/*
					 * echo "FontRecord for sting at $row,$col<br>"; echo str_replace("\n","<br>\n",ExcelFont::toString($font,$fonti));
					 */
					if(!isset($data['max_row']) || ($data['max_row'] < $row))
						$data['max_row']=$row;
					if(!isset($data['max_col']) || ($data['max_col'] < $col))
		...
		while((ord($wb[$ptr]) != 0x0a) && ($ptr < $wblen)){
			...
			switch($opcode){
				...
				case EXCEL_FONT_RID:
					...
					/*
					 * echo str_replace("\n","<br>\n",ExcelFont::toString($rec,count($this->fonts)-1)); echo "FontRecord<br>"
					 */
					;
					break;
				// XF
	...
	/*
	 * 从字符串解析和从文件解析 --------------------------------- IN: string contents - File contents string filename - File name of an existing Excel file. OUT: 0 - success 1 - can't open file 2 - file too small to be an Excel file 3 - error reading header 4 - error reading file 5 - This is not an Excel file or file stored in < Excel 5.0 6 - file corrupted 7 - data not found 8 - Unsupported file version
	 */
	...
	function InitParser(){
		...
		for($i=0; $i < count($header_sig); $i++)
			if($header_sig[$i] != ord($hdrbuf[$i])){
				$this->dbglog->error("InitParser()function founds invalid header"); /* DBG */
				$this->dbglog->error("InitParser()function returns 5"); /* DBG */
				return 5;
			}
...
function setEncode($str,$sCode='UTF-16LE',$tCode='utf-8'){
	// $ret='';
		...
		for($j=0; $j <= $ws['max_row']; $j++){
			for($i=0; $i <= $ws['max_col']; $i++){
				...
				switch($data['type']){
					case 0: // string
						$ind=$data['data'];
						$s=$exc->sst['unicode'][$ind] ? setEncode($exc->sst['data'][$ind]) : $exc->sst['data'][$ind];
						$V=strlen(trim($s)) == 0 ? '' : $s;
						break;
...
function Create_Excel_File($ExcelFile,$Data){
	...
	for($i=0; $i < count($Data[0]); $i++){
		for($j=0; $j < count($Data); $j++){
			$v=setEncode($Data[$j][$i], 'utf-8', 'gb2312');
			xlsWriteLabel($j, $i, $v);
		}
	}
	xlsEOF();
