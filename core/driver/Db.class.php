...
class Db{
	...
	public static function getlimit($page,$pagesize,$total,$prefix=true){
		if($total < $pagesize){
			$page_count=1;
		}else{
			if(!($total % $pagesize)){
		...
		if($total <= $pagesize){
			return ($prefix?'limit ':'').'0,' . $total;
		}
		return ($prefix?'limit ':'') . ($page - 1) * $pagesize . ',' . $pagesize;
	}
	...
	/*
	 * 功能：修改查询表的前置符，支持单表或多表处理 说明：多表格式示例=》 =》tbcommodities,tbcommodities.tbshopid=tbshops.id|left
	 */
	...
	/*
	 * 功能：修正数据库字段信息 说明：能够处理单表，多表，函数（单参数），别名
	 */
	...
	/*
	 * 功能:恢复数据库 参数:restore($fPath='',$mType=''); 说明:支持两种恢复方式: 1.从文件恢复.默认方式,$fPath为文件名,$mType为file或不设置; 2.从字符串恢复.$fPath为字符串,$mType为text;
	 */
	...
	/*
	 * 功能:备份数据库 参数:backup($tb='',$tp='all',$file=''); 说明: 1.文件名选择:如果不提供$file参数,自动搜索所有参数，匹配'/[\/\\]/'即识别为路径； 2.备份类型选择:$tp为struct时只导出表结构；$tp为data时只导出表数据；$tp为all时导出表结构和数据; 3.备份表选择:$tb参数不匹配'/[\/\\]/'即识别为表名； 4.备份数据库类型:根据系统参数，自动识别数据库类型； 5.备份数据处理方式:file型数据库-直接放回；为web型数据库-提供文件保存路径，写入文件，否则返回字符串
	 */
	...
	protected function filterData($data){
		...
		if((is_string($this->mFilterHandle) && function_exists($this->mFilterHandle)) || is_array($this->mFilterHandle)){
