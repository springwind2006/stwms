...
define('SITE_URL', SITE_PROTOCOL . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : '')) . (SITE_PROTOCOL=='https://'?'':SITE_PORT)); /* 网站首页地址 */
...
class load{
	...
	/**
	 * 加载数据库对象 可以用以下方式使用： 方式1：($dbConn array 配置数组,$isNew=1) 方式2：($dbConn string 配置名称,$dbName string,$isNew=1) 方式3：($dbConn string 配置名称,$dbName string,$dbPath='dbase' string,$isNew=1)
	 *
	...
	public static function db($dbConn=NULL,$dbName='',$dbPath='dbase',$isNew=0){
		...
		if($dbConn == 'xml'){
			...
			if(empty($dbConn) || (is_array($dbConn) && !isset($dbConn['type']))){
				...
				$config=load::cfg('database', APP_LOCAL&&($db_conn=='mysql'||$db_conn=='mysqli')?$db_conn.'_loc':$db_conn);
	...
	public static function plugin($name,&$config=NULL){
		...
		if(isset($plugins[$name])){
			$config=$plugins[$name];
			$classname=strpos($name, '.')!==false ? str_replace('.', '_', $name) : 'plugin_' . $name;
			return self::cls($classname, 1, $config, 'plugin' . CD . $config['install_dir'], '');
		}
		return false;
	}
	...
	public static function cfg($file,$key='',$default='',$reload=false){
		...
		if(is_file($path)){
			try{
				...
				if($file == 'system' && defined('ROUTE_M')){
					$default_m=defined('STYLE_MODULE') ? STYLE_MODULE : ROUTE_M;
