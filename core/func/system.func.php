...
/**
 * 根据类路径，获取此类定义的公共方法
 * @param unknown $filename 类路径
 * @param unknown $excludes 排除的方法前缀，为数组或“,”分隔的字符串
 * @param unknown $includes 必须包含的方法前缀，为数组或“,”分隔的字符串
 * @return string[]
 */
function get_class_public_funcs($filename,$excludes=null,$includes=null){
	$class_methods=array();
	if(is_file($filename)){
		$content=preg_replace(array('/\/\/.*?$/m','/\/\*.*?\*\//s'), '', file_get_contents($filename));
		if(is_string($excludes) || is_array($excludes)){
			if(is_string($excludes)){
				$excludes=explode(',', $excludes);
			}
			$excludes=array_unique(array_filter($excludes));
		}
		if(is_string($includes) || is_array($includes)){
		...
		if(preg_match_all('/}\s*(?:public)?\s+function\s+(\w*)\s*\(/is', $content, $matches)){
...
function template($template='index',$dir='',$style='',$module='',$useType=-1){
	...
	if($tplExists){
		if(!is_file($compiledtplfile) || (@filemtime($templatefile) > @filemtime($compiledtplfile)) || (APP_DEBUG && defined('TEMPLATE_REPARSE') && TEMPLATE_REPARSE)){
...
/**
 * 导入静态文件路径 如：'js/show.js@daoke:home'，则返回statics/home/daoke/js/show.js
 *
...
function import($file,$type='',$check=false,$default='default'){
	if(($pos=strpos($file, '/')) !== 0){
		...
		if($module=='plugin'&&isset($GLOBALS[PLUGIN_ID])){
			$style=$GLOBALS[PLUGIN_ID];
		}
		
		if($file === ''){
			return '';
		}
		if($ext=($type !== '' ? $type : fileext($file))){ // 单个文件导入，根据文件名（如果为js,css,images文件）自动识别上级目录
		...
		if($check){
			if(!is_file(STATIC_PATH . $module . CD . ($style === '' ? '' : $style . CD) . $file)){
				return is_file(STATIC_PATH . $module . CD . $default . CD . $file) ? STATIC_URL . $module . '/default/' . $file : '';
			}
		}
