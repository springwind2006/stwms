...
$.formValidator = 
{
	...
	{
		var settings = {},initConfig=initConfig_setting();
		$.extend(true, settings, initConfig, controlOptions || {});
    	...
    	if(settings.theme!=initConfig.theme){
			$.ajax({async:false,type: "GET",cache:true,url: themedir + "themes/"+settings.theme+"/js/theme.js",dataType: "script",
				error :function(){alert(fv_load_lang("onerror_load_theme",{theme:settings.theme}))}
			}); 
		}
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		else
		{
			...
			else
			{
				...
				if(showclass!=""){
					tip.html(html).removeClass().addClass(showclass);/*.show();*/ /*[Modify]*/
				}else{
					tip.html(html);/*.show();*/ /*[Modify]*/
				}
			}
			var stype = elem.type;
	...
	pageIsValid : function (validatorGroup)
	{
		...
		if(isValid && initConfig.debug){alert(fv_load_lang("onerror_debug_submit"));}
	...
	ajaxForm : function(validatorGroup,option,formid)
	{
		...
		if(formid==undefined){
			...
			if(formid==""){alert(fv_load_lang("onerror_no_formid"));return false;};
		};
	...
	reloadAutoTip : function(validatorGroup)
	{
		...
		$.each(initConfig.validObjects,function()
		{
			...
			if(initConfig.mode == "AutoTip" && setting.bind== true)
			{
				...
				if(typeof(offset.top)!="undefined" && typeof(offset.left)!="undefined" && (offset.top!=0 || offset.left!=0)){
					var y = offset.top;
					var x = $(relativeID ).width() + offset.left;
					$("#"+setting.tipID).show().css({left: (x+10)+"px", top: (y-4)+"px"});
				}else{
					$("#"+setting.tipID).hide();
				}		
			}
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
		    ...
		    jqobj.bind("keyup",function(){
		        ...
		        if(this.validatorPasswordIndex > 0)
		        {
					...
					{
						alert(fv_load_lang("onerror_password_check"));
					}
	            }
		    });
...
var initConfig_setting = function(){
	return {
		theme:"Default",
		...
		ajaxPrompt : fv_load_lang("ajax_prompt"),	//控件失去焦点后，触发ajax校验，没有返回结果前的错误提示数
...
var formValidator_setting = function(){
	return {
		validatorGroup : "1",
		onShowText : "",
		onShowTextColor:{mouseOnColor:"#000000",mouseOutColor:"#999999"},
		onShowFixText:"",
		onShow :fv_load_lang("onshow"),
		onFocus: fv_load_lang("onfocus"),
		onCorrect: fv_load_lang("oncorrect"),
		onEmpty: fv_load_lang("onempty"),
		empty :false,
		autoModify : false,
		defaultValue : null,
		bind : true,
...
var inputValidator_setting = function(){
	return {
		isValid : false,
		type : "size",
		min : 0,
		max : 99999,
		onError:fv_load_lang("onerror"),
		validateType:"inputValidator",
		empty:{leftEmpty:true,rightEmpty:true,leftEmptyError:null,rightEmptyError:null}
	};
}

var compareValidator_setting = function(){
	return {
		isValid : false,
		desID : "",
		operateor :"=",
...
var regexValidator_setting = function(){
	return {
		isValid : false,
		regExp : "",
		param : "i",
		dataType : "string",
		compareType : "||",
		onError:fv_load_lang("onerror_reg"),
		validateType:"regexValidator"
	};
}
var ajaxForm_setting = function(){
	return {
		type : "GET",
...
var ajaxValidator_setting = function(){
	var cfg={
		isValid : false,
		lastValid : "",
		oneceValid : false,
		randNumberName : "rand",
		onError:fv_load_lang("onerror_ajax"),
		onWait:fv_load_lang("onwait"),
		validateType:"ajaxValidator"
	};
	$.extend(true,cfg,ajaxForm_setting());
	return cfg;
}


var functionValidator_setting = function(){
	return {
		isValid : true,
...
var passwordValidator_setting = function(){
	return {
		isValid : true,
		compareID : "",
		validateType:"passwordValidator",
		onErrorContinueChar:fv_load_lang("onerror_continue_char"),
		onErrorSameChar:fv_load_lang("onerror_same_char"),
		onErrorCompareSame:fv_load_lang("onerror_compare_same")
	};
}


var fv_get_para=function(url,ky,dft){
	if(url.indexOf("?")!=-1){
...
var fv_load_lang=function(ky,para){
	if(typeof(ky)=="undefined"){
		var scriptSrcArray = fv_scriptSrc.split('/'),
				jsName = scriptSrcArray[scriptSrcArray.length-1],
				rootdir = fv_scriptSrc.replace(jsName,''),
				lang=fv_get_para(fv_scriptSrc,'lang',0);
		if(lang){
			jQuery.ajax({async:false,cache:true,type: "GET",url: rootdir + "lang/"+lang+".js",dataType: "script",
				error :function(){
					jQuery.ajax({cache:true,type: "GET",url: rootdir + "lang/en.js",dataType: "script"});
				}
			});
		}
	}else{		
		var cLang=typeof(Formvalidator_LANG)!="undefined"&&typeof(Formvalidator_LANG[ky])!="undefined" ? Formvalidator_LANG[ky] : ky;
...
var fv_load_theme=function(){
		var scriptSrcArray = fv_scriptSrc.split('/');
		var jsName = scriptSrcArray[scriptSrcArray.length-1];
    var themedir = fv_scriptSrc.replace(jsName,'');
    var themeCfg = initConfig_setting();
    var theme=fv_get_para(fv_scriptSrc,'theme',themeCfg['theme']);    	
		$.ajax({async:false,type: "GET",cache:true,url: themedir + "themes/"+theme+"/js/theme.js",dataType: "script",
			error :function(){alert(fv_load_lang("onerror_load_theme",{theme:theme}))}
		});	
}
fv_load_lang();
fv_load_theme();
