...
class CategoryAdmin extends AdminController{
	...
	function init(){
		...
		foreach($result as $k=>$r){
			...
			$result[$k]['count']=$r['type'] == 0 && empty($r['arrcid']) ? $this->statistics($r['model'], $r['type']) : '-';
	...
	public function add(){
		if(isset($_POST['dosubmit'])){
			...
			chdir(CORE_PATH . 'template' . CD . 'home' . CD);
	...
	public function edit(){
		...
		if(isset($_POST['dosubmit'])){
			...
			foreach($deIds as $id){
				...
				$allcategorys[$id]['hasDatas']=$this->statistics($categorys[$id]['model'], $categorys[$id]['type']);
				...
				if(!empty($categorys[$id]['setting']['template_style'])){
					chdir(CORE_PATH . 'template' . CD . 'home' . CD . $categorys[$id]['setting']['template_style']);
	...
	public function del(){
		...
		foreach($deIds as $id){
			$id=intval($id);
			if($this->statistics($categorys[$id]['model'], $categorys[$id]['type'])){
				continue;
			}
	...
	public function _get_template($cfg){
		$template_styles=array();
		if(!empty($cfg['style']) && is_dir(CORE_PATH . 'template' . CD . 'home' . CD . $cfg['style'] . CD)){
			chdir(CORE_PATH . 'template' . CD . 'home' . CD . $cfg['style'] . CD);
	...
	private function statistics($table,$type=0){
		if(!$type && in_array($this->getDb()->mTb($table), $this->getDb()->tables())){
			return $this->getDb()->count($table);
		}
		return 0;
	}
}
