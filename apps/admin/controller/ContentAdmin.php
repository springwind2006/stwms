...
class ContentAdmin extends AdminController{
	...
	public function manage(){
		...
		if(!$categorys[$catid]['type'] && !empty($categorys[$catid]['model'])){
			...
			$condtions='where `status`' . ($istrash ? '=0' : ($audit_levels?'!=0':'=1'));
			...
			if(array_key_exists($_GET['s_type'], $fields) && $s_kw !== ''){
				if($fields[$_GET['s_type']]['msetting']['iseqsearch']){
					$condtions.=' and `' . $_GET['s_type'] . '` = \'' . $s_kw . '\'';
				}else{
	...
	private function getOutputFormat($field,$v,$position_field,$dx,$s_kw,$searchColor='red',$thumbHeight=15){
		...
		switch($field['formtype']){
			...
			case 'image':
				$maxchars=0;
				if(strpos($v[$field['field']], SITE_URL)===0||strpos($v[$field['field']], UPLOAD_URL)===0){
					...
					$val='<a title="点击查看'.str_replace(array("\r","\n",'"'),'',$field['name']).'" style="cursor:pointer;" onclick="view_images(\'' . $v[$field['field']] . '\')" >点击查看</a>';
				}
				break;
			case 'images':
				$maxchars=0;
		...
		if(isset($_GET['s_type']) && $_GET['s_type'] == $dx && !empty($s_kw) && !$field['msetting']['iseqsearch']){
