...
class MenuAdmin extends AdminController{
	...
	private function getCls(){
		$module=ucfirst(strtolower(defined('STYLE_MODULE') ? STYLE_MODULE : ROUTE_M));
		$clsArr=glob(dirname(__FILE__) . CD . '*' . $module . '.php');
	...
	private function getAct($c){
		...
		$name=ucfirst($c) . ucfirst(strtolower($default_m));
		$file=CORE_PATH . $path . CD . (is_file(CORE_PATH . $path . CD . 'MY_' . $name . '.php') ? 'MY_' : '') . $name . '.php';
		$class_methods=get_class_public_funcs($file,'_');
		foreach($class_methods as $k=> $method){
			$class_methods[$k]=array('a' => $method,'isuse' => ($this->check($c, $method) ? 1 : 0));
		}
