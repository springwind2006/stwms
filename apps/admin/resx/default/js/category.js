$(function() {
	...
	if (ROUTE_A == 'add' || ROUTE_A == 'edit') {
		var getCid = function(idstr) {
			return idstr.substr(idstr.lastIndexOf("_"));
		};
		...
		$("input[id^='type_']").each(function() {
			...
			if (ctp == "0") {
				...
				if (ROUTE_A == 'edit') {
					$("#model" + cid).defaultPassed();
				}
			}
			$("#name" + cid).formValidator({
				onShow : "请输入栏目名称"
			}).inputValidator({
			    min : 1,
			    onError : "不能为空,请确认"
			});
			$("#url" + cid).formValidator({
				onShow : "请输入外链地址"
			}).inputValidator({
			    min : 1,
			    empty : {
			        leftEmpty : false,
			        rightEmpty : false,
			        emptyError : "类型名称两边不能有空符号"
			    },
			    onError : "不能为空,请确认"
			});

			$("#cdir" + cid).formValidator({
				onShow : "请输入英文目录"
			}).inputValidator({
			    min : 1,
			    empty : {
			...
			$("#simple_url" + cid).formValidator({
				onShow : "为空则不简化地址"
			}).regexValidator({
			    regExp : "(?:^\\w[\\w/]+/$)|(?:^\\w[\\w/]+\\.php$)|(?:^$)",
			    onError : "填写目录名(以/结尾)或文件名(以.php结尾)"
			}).ajaxValidator({
			    type : "get",
			...
			$("#template_style" + cid).change(function() {
				...
				if (cStyle == "") {
					...
					$.ajax({
					    dataType : "json",
					    url : act_url("category", "init", "get_template=1&style=" + cStyle + "&type=" + cType + "&mcid=" + cid),
					    success : function(obj) {
						    var cid = obj["mcid"];
						    $.each(obj, function(dx, vl) {
							    if (dx == "mcid"||typeof(vl)!="object")
								    return true;
							    var sels = "";
			...
			$("#category_ishtml_0" + cid + ",#category_ishtml_1" + cid + ",#show_ishtml_0" + cid + ",#show_ishtml_1" + cid).click(function() {
				var mid = $(this).attr('id'), cid = getCid(mid), tp = mid.substr(0, mid.indexOf("_")), isShow = (mid.substr(mid.indexOf("ishtml") + 7, 1));
		...
		$("#bat_act").click(function() {
			var act_type = $("#act_type").val(), selLen = $("input[name^='id']:checked").length;
			if (selLen < 1) {
