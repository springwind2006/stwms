$(function(){
	var resize=function(){
		var ah=$(document).height()-50;
		$("#leftMenu").height(ah);
		$("#centerMenu").height(ah-28);
		$("#mainContent").height(ah-28);
		$("#mainContent,#mainNav").width($(window).width()-180);
	}
	...
	$(".navmenu >li").click(function(){
		...
		$(".navmenu >li").removeClass("open-li expanded");
		$(this).addClass("open-li expanded");
		if(typeof(url)!="undefined"){
			doMainFrame.location=url;
		}else{
			$(".navmenu >li .angle").removeClass("haschild");
			$(".navmenu >ul").height(0);
			$(".angle",this).addClass("haschild");
			$("+ul",this).height($("+ul>li",this).length*30);
		}
	});
	$(".navmenu >ul>li").click(function(){
		$(".navmenu >ul>li").removeClass("selected");
		$(this).addClass("selected");
		var mainname=$(".menutext",$(this).parent().prev(".nav")).html();
		var subname=$(".menutext",this).html();
		$("#opNavLeft").html("当前操作："+mainname+" &gt; "+subname);
		var href=$(this).attr("data-url");
		if(typeof(href)!="undefined"){
			doMainFrame.location=href;
		}
	});
...
function closeCenterMenu(){
	$("#centerMenu").hide();
	$("#mainContent").css("marginLeft",180);
	$("#mainContent").width($(window).width()-180);
}
function openCenterMenu(){
	$("#centerMenu").show();
	$("#mainContent").css("marginLeft",360);
	$("#mainContent").width($(window).width()-360);
}
