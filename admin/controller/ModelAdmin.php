...
class ModelAdmin extends AdminController{
	...
	public function import(){
		if(Param::get_para('dosubmit')){
			unset($_POST['attr']['type']);
			$attr=safe_replace($_POST['attr']);
			$tbname=preg_replace("/[^\w]/", '', $attr['tbname']?$attr['tbname']:basename($_POST['Filename'],'.model'));
			...
			if(move_uploaded_file($_FILES['Filedata']['tmp_name'], $tmpPath)){
				...
				if(!empty($attr) && is_array($attr)){
					foreach ($attr as $k=>$v){
						if(!empty($v)){
							$this->getXmlDb()->schema('model', $k, $v);
						}
					}
				}
				$this->_fresh();
	...
	public function export(){
		...
		if(copy($sourcePath, $tmpPath)){
			...
			header('Content-Disposition:filename='.$tbname . '.model');
			$fp=fopen('php://output', 'a');
			fwrite($fp, file_get_contents($tmpPath));
	...
	private function get_count($model){
		return $this->getDb()->count($model);
	}
