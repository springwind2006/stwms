...
function toogle_status(id, domobj, field, table) {
    $.ajax({
        url: ROOT + "?" + VAR_MODULE + "=" + MODULE_NAME + "&" + VAR_ACTION + "=toogle_status&table=" + table + "&field=" + field + "&id=" + id,
        data: "ajax=1",
        dataType: "json",
...
function toogle_sponsor_status(id, htmlobj) {
    var url = ROOT + "?" + VAR_MODULE + "=" + MODULE_NAME + "&" + VAR_ACTION + "=toogle_sponsor_tatus&id=" + id;
    $.ajax({
        url: url,
        dataType: "json",
        type: "GET",
        success: function (ajaxobj) {
            if (ajaxobj.data == '1') {
                $(htmlobj).html(LANG['YES']);
            } else {
                $(htmlobj).html(LANG['NO']);
            }
        }
    })
}
...
function load_balance(id)
{
    ...
    $.ajax({
        ...
        success: function (obj) {
            if (obj.status)
            {
                $.weeboxs.open(ROOT + '?c=Balance&a=load_balance&id=' + id + "&deal_id=" + deal_id, {contentType: 'ajax', showButton: false, title: LANG['DO_BALANCE'], width: 600, height: 200});
            }
            else
            {
                alert(obj.info);
...
//专属话题回复
function reply(id) {
    location.href = ROOT + "?" + VAR_MODULE + "=" + MODULE_NAME + "&" + VAR_ACTION + "=topic_edit&id=" + id;
}
function add_reply(topic_id) {
    var ajaxurl = ROOT + "?" + VAR_MODULE + "=" + MODULE_NAME + "&" + VAR_ACTION + "=topic_edit&id=" + topic_id;
    $.ajax({
        url: ajaxurl,
        dataType: "json",
        type: "POST",
        success: function (ajaxobj) {
            $('#topic_content').after(ajaxobj.html);
        }
    });
}

function reply_submit() {
    $('#reply_form').bind('submit', function () {
        var query = {};
        $("textarea.add_reply").each(function (i, val) {
            query['content' + i] = val.value;
        });
        var topic_id = $('input[name=topic_id]').val();
        var ajaxurl = ROOT + "?" + VAR_MODULE + "=" + MODULE_NAME + "&" + VAR_ACTION + "=topic_edit&id=" + topic_id + "&type=add";
        $.ajax({
            url: ajaxurl,
            type: "POST",
            dataType: "json",
...
function del_topic(id) {
    if (window.confirm("您确定要删除吗？删除后不可以恢复！")) {
        window.location = ROOT + '?c=Project&a=del_topic&id=' + id;
    }
}