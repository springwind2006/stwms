...
class PluginAdmin extends AdminController{
	...
	/*
	 * 功能：安装插件 说明：安装过程根据提交参数安装过程分为上传、解压、安装、清理，同时前台界面显示安装进度 服务器返回码： 0->上传文件失败； 1->不允许上传的文件类型； 2->解压失败； 3->解压成功； 4->插件已经存在； 5->插件安装失败； 6->插件安装成功； 7->重复安装插件； 8->测试运行失败；
	 */
	public function install(){
		if(Param::get_para('dosubmit')){
			@set_time_limit(0); // 修改为不限制超时时间(默认为30秒)
			$is_debug=false; //是否启用调试
					...
					if(!is_file($zipfile)){
						...
						if(function_exists('zip_open')){
								...
								while($zip_entry=zip_read($zip)){
									...
									if(substr($filename,-1,1)!='/'){
										if(zip_entry_open($zip, $zip_entry)){
											...
											$filename=rtrim($savepath,'/').'/'.$filename;
											$pathname=dirname($filename);
											!is_dir($pathname) && mkdir($pathname,0777,true);
											file_put_contents($filename, zip_entry_read($zip_entry,zip_entry_filesize($zip_entry)));
							...
							for($i=0, $max=count($array); $i < $max; $i++){
								if(strpos($array[$i]['filename'], '__MACOSX/')===0){
									continue;
								}
								if($array[$i]['folder'] == 0){
				...
				case 'install': // 安装
					...
					if(is_array($cfg) && !empty($cfg['name']) && isset($_SESSION[$sourceName]) && $_SESSION[$sourceName] == $md5str){
						...
						if(!is_dir(CORE_PATH . 'plugin' . CD . $install_dir)){
							...
							if($resMove){
								$controller=load::cls('plugin_' . $cfg['name'], 0, NULL, 'plugin' . CD . $install_dir,'');
								if($controller){
									$installDbOk=true;
									// 后续操作一：安装数据库
							...
							$is_debug && lg("安装目录已经存在\r\n",'log.txt',1);
							echo '4'; // 安装目录已经存在，安装失败
						}
					}else{
						$is_debug && lg("安装配置文件不存在\r\n",'log.txt',1);
	...
	public function export(){
		...
		if(mkdir($tempPath, 0777, true) && dir_copy($core_path, $tempPath)){
			if(is_dir($static_path) && mkdir($tempPath . 'statics' . CD, 0777, true) && dir_copy($static_path, $tempPath . 'statics' . CD)){
				// 对模型的处理
		...
		if($c && isset($pluginsMap[$c])){
			...
			if($controller){
				...
				if(method_exists($controller, $method)){
					...
					!defined('STWMS_TMPL_C') && define('STWMS_TMPL_C','admin');
	...
	// 调用插件，提供给插件标签处的调用，应用于前端代码
	public function _call($datas=array(),$method_pre='_tag_'){
		$c=$datas['name'];
		$method=$method_pre . (isset($datas['action']) && $datas['action'] ? $datas['action'] : 'index');
		$controller=load::plugin($c, $cfg);
		if($controller && method_exists($controller, $method)){
			$GLOBALS[PLUGIN_ID]=$cfg['install_dir'];
			!defined('STWMS_TMPL_C') && define('STWMS_TMPL_C',defined('ADMIN_INI')?'admin':'user');
			!defined('STWMS_TMPL_A') && define('STWMS_TMPL_A',$a);
	...
	private function get_plugin_methods($cfg){
		$filename=PLUGIN_PATH . $cfg['install_dir'] . CD . 'plugin_' . $cfg['name'] . '.php';
		return get_class_public_funcs($filename,'_,m_');
	}
}
