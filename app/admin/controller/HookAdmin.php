...
class HookAdmin extends AdminController{
	...
	public function init(){
		...
		if(Param::get_para('get_plugin_a')){
			...
			$class_methods=array();
			if(isset($plugins[$plugin_c])){
				$filename=CORE_PATH . 'plugin' . CD . $plugins[$plugin_c]['install_dir'] . CD . 'plugin_' .$plugin_c . '.php';
				if(is_file($filename)){
					$info=$this->getXmlDb()->getOne('hook', 'id,name,data', 'where `id`=' . $this->id);
					$name=$info['name'];
					unset($info);
					$maps=getcache('hook', 'core', 'array', 'map');
					$class_methods=get_class_public_funcs($filename);
					foreach($class_methods as $k=> $method){
						if(isset($maps[$name]) && strpos($maps[$name] . ',', $plugin_c . '.' . $method . ',') !== false){
							unset($class_methods[$k]);
						}
					}
				}
			}
			alert(array_values($class_methods));
		}
