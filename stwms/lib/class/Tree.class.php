...
class Tree{
	...
	/**
	 * 功能:同上一类方法，jquery treeview 风格，可伸缩样式（需要treeview插件支持）
	 * 参数：get_treeview( $myid 表示获得这个ID下的所有子级,
	 * $effected_id 需要生成treeview目录数的id,
	 * $str 末级样式,
	 * $str2 目录级别样式,
	 * $showlevel 直接显示层级数，其余为异步显示，0为全部显示,
	 * $style 目录样式 默认 filetree 可增加其他样式如'filetree treeview-famfamfam',
	 * $currentlevel 计算当前层级，递归使用 适用改函数时不需要用该参数,
	 * $recursion 递归使用 外部调用时为FALSE
	 * )
	 */
	...
	/**
	 * 功能：获取子栏目json
	 * 参数：creat_sub_json($myid 当前id, $str 字符串)
	 */
