...
final class TemplateCache{
	...
	public function template_parse($str){
		...
		//全局标签的解析
		stripos($str, $ld . 'action ') !== false && ($str=preg_replace_callback('/' . $ld . 'action\s+(.+?)\s*\/?' . $rd . '/i', array($this,'parseAction'), $str));
		stripos($str, $ld . 'import ') !== false && ($str=preg_replace_callback('/' . $ld . 'import\s+(.+?)\s*\/?' . $rd . '/i', array($this,'parseImport'), $str));
		(stripos($str, $ld . 'template ') !== false || stripos($str, $ld . 'include ') !== false) && ($str=preg_replace_callback('/' . $ld . '(?:template|include)\s+(.+?)\s*\/?' . $rd . '/i', array($this,'parseInclude'), $str));
		if(stripos($str, $ld . 'mc:') !== false || stripos($str, $ld . 'pc:') !== false){
			$str=preg_replace_callback('/' . $ld . '(?:p|m)c:(\w+)\s+(.+?)\s*\/?' . $rd . '/i', array($this,'parseMc'), $str);
			$str=preg_replace('/' . $ld . '\/(?:p|m)c.*?' . $rd . '/i', '', $str);		
		}
		
		//PHP标签的解析
		if(stripos($str, $ld . 'php') !== false){
			//单行PHP代码；php标签内不能带有'{'或'}'字符
			$str=preg_replace('/' . $ld . 'php\s+([^' . $ld . '' . $rd . ']+)\s*\/?' . $rd . '/i', '<?php \\1;?>', $str); 
			//多行PHP代码
		...
		//条件标签的解析（支持“.”语法表示数组）
		stripos($str, $ld . 'if ') !== false && ($str=preg_replace_callback('/' . $ld . 'if\s+(.+?)\s*' . $rd . '/i', array($this,'parseIf'), $str));
		stripos($str, $ld . 'else') !== false && ($str=preg_replace('/' . $ld . 'else\s*\/?' . $rd . '/i', '<?php } else { ?>', $str));
		stripos($str, $ld . 'elseif ') !== false && ($str=preg_replace_callback('/' . $ld . 'elseif\s+(.+?)\s*\/?' . $rd . '/i', array($this,'parseElseif'), $str));
		stripos($str, $ld . '/if') !== false && ($str=str_ireplace($ld . '/if' . $rd, '<?php } ?>', $str));
		
		//////////////循环的解析/////////////
		// for 循环
		if(stripos($str, $ld . 'for ') !== false){
			$str=preg_replace('/' . $ld . 'for\s+(.+?)' . $rd . '/i', '<?php for(\\1) { ?>', $str);
		...
		if(stripos($str, $ld . 'loop ') !== false || stripos($str, $ld . 'foreach ') !== false){
			$str=preg_replace_callback('/' . $ld . '(?:loop|foreach)\s+(.+?)' . $rd . '/i', array($this,'parseForeach'), $str);
			$str=preg_replace('/' . $ld . '\/(?:loop|foreach)' . $rd . '/i', '<?php } ?>', $str);
		}
	...
	public function parseInclude($matches){
		...
		if(isset($arrs['file'])){
			...
			foreach($files as $file){
				...
				$paras=array_reverse(explode('/', $file,3));
				...
				if($module!==''){
					$len=count($paras);
					$len==1&&array_push($paras, '\'\'', '\'\'');
					$len==2&&array_push($paras, '\'\'');
					array_push($paras, '\''.$module.'\'');
				}
				if($dtype){
	...
	public function parseForeach($matches){
		...
		if(isset($arrs['name']) && isset($arrs['item'])){
			...
			return '<?php '.$order.'=0;if(is_array('.$arrs['name'].')) foreach('.
					$arrs['name'].' as '.
					(isset($arrs['key']) ? $arrs['key'].'=>'.$arrs['item']:$arrs['item']).
					') { '.$order.'++;?>';
		}
	}
