...
/*解析Parse workbook*/
	function parse_workbook($f_header,$dp){
		...
		for($i=0; $i < $num_fat_blocks; $i++){
			...
			for($j=0; $j<0x80; $j++)$this->fat[]=$fat->getLong($j * 4);
			$fat->close();
		...
		if($small_block != 0xfeffffff){
			...
			if($root_entry_index < 0){
			 ...
			 for($i=0; $i < count($schain); $i++){
				  ...
				  for($j=0; $j<0x80; $j++)$this->sfat[]=$sfat->getLong($j * 4);
				  $sfat->close();
				  unset($sfatbuf,$sfat);
			 }
			 unset($schain);
					  ...
					  if($sst_defined)return 6;
						...
						if($rslen > $slen){
								...
								if((ord($wb[$sptr])& 1)> 0){
									if($char_bytes == 1){
										$sstr='';
										for($i=0; $i< strlen($str); $i++)$sstr .= $str[$i].chr(0);
										$str=$sstr;
										$char_bytes=2;
									}
									$schar_bytes=2;
...
function Create_Excel_File($ExcelFile,$Data){
	...
	function xlsWriteNumber($Row,$Col,$Value){echo pack("sssss",0x203,14,$Row,$Col,0x0);echo pack("d",$Value);return;}
