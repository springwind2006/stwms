...
class Controller{
	...
	private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false){
		...
		is_int($ajax) && $this->assign('waitSecond', $ajax);
		!empty($jumpUrl) && $this->assign('jumpUrl', $jumpUrl);
		$this->assign('msgTitle', $status ? '操作成功！' : '操作失败！');
		$this->get('closeWin') && $this->assign('jumpUrl', 'javascript:window.close();');
		$this->assign('status', $status); // 状态
		$this->assign('message', $message); // 提示信息
		if($status){
			!isset($this->waitSecond) && $this->assign('waitSecond', '1');
			!isset($this->jumpUrl) && $this->assign('jumpUrl', $_SERVER['HTTP_REFERER']);			
			$this->display(C('TMPL_ACTION_SUCCESS','/message'));
		}else{
			$this->assign('error', $message);
			!isset($this->waitSecond) && $this->assign('waitSecond', '3');
			!isset($this->jumpUrl) && $this->assign('jumpUrl', 'javascript:history.back(-1);');
