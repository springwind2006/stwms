...
class AccessDb extends Db implements DbInterface{
	...
	public function connect($dbConfig){
		...
		try{
			...
			$conn_strs=array(
						'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=' . realpath($dbConfig['DSN']) . ';Jet OLEDB:Database Password=' . $dbConfig['pass'],
						'DRIVER={Microsoft Access Driver (*.mdb)};dbq=' . realpath($dbConfig['DSN']) . ';uid=' . $dbConfig['user'] . ';pwd=' . $dbConfig['pass']
					);
