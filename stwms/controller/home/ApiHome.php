...
class ApiHome extends HomeController{
	 ...
	 * 参数说明：
	 * 提交方式：GET；
	 * bg:背景颜色，值为6位16进制字符，如:ff0000(红色)；
	 * ft:字体颜色，与bg参数相同；
	 * w:图片宽度，为数值，默认为50，单位像素；
	 * h:图片高度，为数值，默认为20，单位像素；
	 * num:验证码位数：默认为5
	 */
	 ...
	 * 功能：直接处理图片，不缓存至文件
	 * 参数示例：
	 * 本地图片-》index.php?c=api&a=thumb&f=/ufs/2013/1013/20131013125942100.jpg&w=600&h=500&cut=1
	 * 网络图片-》index.php?c=api&a=thumb&f=http://p8.qhimg.com/t01e87328014671ce55.jpg&w=600&h=500
	 * 参数说明：
	 * 提交方式：GET；
	 * f:图片地址，为相对于服务器的地址；
	 * w:图片最大宽度，默认为0，即按原宽度显示；
	 * h:图片最大高度，默认为0，即按原高度显示；
	 * cut:是否剪裁图片，默认为0，即不剪裁，剪裁图片后输出图片完全按照给定的高度和宽度显示图片
	 */
	 ...
	 * 功能：下载文件
	 * 参数示例：
	 * 1.index.php?c=api&a=download&f=L3Vmcy8yMDEzLzEwMTMvMjAxMzEwMTMxMjU5NDIxMDAuanBn
	 * 2.index.php?c=api&a=download&fid=12
	 * 参数说明：
	 * 提交方式：GET；
	 * f:图片地址，已经用base64加密，为相对于服务器的地址；
	 * fid:附件在附件库中id
	 */
	 ...
	 * 功能：获取访问次数
	 * 参数：(栏目和ID标识,类别)
	 * 说明：此函数外部调用时需GET提交catid和id参数，内部调用第一个参数为{catid}-{id}格式，
	 * 第二个参数是否同时更新（默认为1）
	 */
