...
class ContentHome extends HomeController{
	 ...
	 * paras参数说明：
	 * 必须属性->action、catid；
	 * 可选属性->where、typeid,posid（根据action调用）
	 */
	 ...
	 * 处理属性说明：
	 * catid：栏目ID,必须填写；
	 * typeid：标签所属分类ID,可以不用设置（默认为0，即获取顶级分类）、为数值
	 * 或$typeid变量（此种方式会随着提交的typeid参数值变化而改变父级分类）。
	 * where：查询条件，默认为空，即查询catid栏目下的所有数据，格式如：“id>10”或“id>10 and name like '%li%'”。
	 * num：数量的限制，数字，默认为空获取所有内容；
	 * start：开始条数，数字，默认为空，从第一条开始；
	 * order：排序，默认空按照id倒序输出，格式如：“listorder desc”或“listorder desc,id asc”；
	 * format：是否进行数据输出格式化，默认为1，反之这直接调用数据库中的原始数据；
	 * thumb：只获取具有缩略图的数据，如果返回的字段不包括“thumb”或“thumb”为空，则从数据所有数据中搜索；
	...
	/**
	 * 功能：hits标签处理函数，用于获取点击排行
	 * 处理属性说明：
	 * catid：栏目ID,默认为空，即调用所有栏目的数据，多个栏目之间用逗号隔开，格式如：“12,25”；
	 * day：调用多少天内的排行，默认为空即调用所有的排行。
	 * limit：数量的限制，默认为空获取所有推荐内容，格式如：“5”或“2,5”，分别表示获取5条或从第三条开始获取五条；
	 * order：排序类型,默认为“views desc”（本月排行- monthviews desc 、本周排行 - weekviews desc、今日排行 - dayviews desc）
	 * format：是否进行数据输出格式化，默认为1，反之这直接调用数据库中的原始数据。
	 */
	 ...
	 * 处理属性说明：
	 * posid：推荐位ID,必须填写；
	 * limit：数量的限制，默认为空获取所有推荐内容，格式为：“5”或“2,5”，分别表示获取5条或从第三条开始获取五条；
	 * format：是否进行数据输出格式化，默认为1，反之这直接调用数据库中的原始数据。
	 * thumb：只获取具有缩略图的数据，如果返回的字段不包括“thumb”或“thumb”为空，则从数据所有数据中搜索；
	 */
